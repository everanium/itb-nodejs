// BLAKE2s-focused Node.js / TypeScript binding coverage.
//
...
// (hash, ITB_seed_width) — BLAKE2s ships only at -256. The width
// feeds into Seed.fromComponents key validation later in this file.
const BLAKE2S_HASHES: ReadonlyArray<readonly [string, number]> = [
  ['blake2s', 256],
];

// Hash-key length (bytes) per primitive — locks in the FFI-surfaced
// contract that BLAKE2s carries a 32-byte fixed key.
...
describe('test_blake2s', () => {
  ...
  test('persistence across nonce sizes', () => {
    ...
    for (const [hashName, width] of BLAKE2S_HASHES) {
