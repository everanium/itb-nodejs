...
// across the three nonce sizes, both Ouroboros modes, and the
// persistence + plaintext-size sweeps.
//
// Each Python TestCase.test_* maps to a single test() here; the
// per-class subTest loops are inlined since node:test has no
// equivalent of unittest subTest. Encryptor.setNonceBits is
// per-instance and does not touch process-global state, so these
// tests do not need cross-process serialisation.
...
// `randomBytes` returns a Buffer (a Uint8Array subclass). The cipher
// methods always emit plain Uint8Array, so plaintext is normalised
// to plain Uint8Array up front to keep `assert.deepEqual` happy on
// the strict prototype check.
function rng(n: number): Uint8Array {
  return new Uint8Array(randomBytes(n));
}

describe('test_easy_aescmac', () => {
