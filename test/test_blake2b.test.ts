// BLAKE2b-focused Node.js / TypeScript binding coverage.
//
...
// into Seed.fromComponents key validation later in this file.
const BLAKE2B_HASHES: ReadonlyArray<readonly [string, number]> = [
  ['blake2b256', 256],
  ['blake2b512', 512],
];

// Hash-key length (bytes) per primitive — locks in the FFI-surfaced
// contract that BLAKE2b256 carries a 32-byte fixed key and
// BLAKE2b512 carries a 64-byte fixed key.
...
describe('test_blake2b', () => {
  ...
  test('persistence across nonce sizes', () => {
    ...
    for (const [hashName, width] of BLAKE2B_HASHES) {
