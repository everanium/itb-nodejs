// Areion-SoEM-focused Node.js / TypeScript binding coverage.
//
// Symmetric counterpart to test_blake2b.test.ts: the same coverage
// shape (nonce-size sweep, single + triple roundtrip, single +
// triple auth with tamper rejection, persistence sweep,
// plaintext-size sweep) applied to areion256 / areion512.
// Areion's batched arm has been shipped longer than BLAKE2b's, but
// the sibling test_nonce_sizes / test_auth files happened to pick
// non-Areion representatives for the width-256 / width-512
// buckets, so dedicated Areion coverage at those axes is added
// here.
//
// What this file adds beyond the parametrised iterators in the
// sibling test files:
//
//   * areion256 + areion512 coverage across all three setNonceBits
//     values (sibling test_nonce_sizes iterates blake3 +
//     blake2b512 + siphash24).
//   * areion256 + areion512 authenticated round-trip including
//     tamper rejection at the dynamic header offset (sibling
//     test_auth picks blake3 + blake2b512 + siphash24).
//   * areion256 + areion512 persistence round-trip cycling
//     nonce_bits across all three values inside a single test —
//     the Areion-SoEM ASM kernels are length-specialised at 20 /
//     36 / 68 byte buf shapes, so the full sweep surfaces any
//     drift between encrypt-time and decrypt-time nonce_bits
//     configuration.
//
// Mirrors bindings/python/tests/test_areion.py one-to-one.
...
// (hash, ITB_seed_width) — both Areion-SoEM widths. The width
// feeds into Seed.fromComponents key validation later in this file.
const AREION_HASHES: ReadonlyArray<readonly [string, number]> = [
  ['areion256', 256],
  ['areion512', 512],
];
...
describe('test_areion', () => {
  ...
  test('persistence across nonce sizes', () => {
    ...
    for (const [hashName, width] of AREION_HASHES) {
