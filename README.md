...
Windows: the same; libitb3 builds as `libitb3.dll`.

## Build the shared library

The convenience driver builds `libitb3.so`, installs npm
...
sender.saveF('session.blob');                     // written by libitb3, mode 0600
...
JSON libitb3 emits; absent keys are optional fields at their zero
value.
...
```ts
import { setGCPercent, setMemoryLimit } from 'libitb3';

setMemoryLimit(4n * 1024n * 1024n * 1024n); // BigInt: the limit is int64
setGCPercent(100);
```
