...
if [[ "${ITB_SKIP_CLEAN:-0}" == "1" ]]; then
    ...
    if [[ "${ITB_KEEP_DOWNLOADS:-0}" == "1" ]]; then
        ...
        # fetched, and a kept one makes tsc skip work the wipe expects.
        clean_tree '*.tsbuildinfo'
    else
        clean_tree '*.tsbuildinfo'
        clean_target 'node_modules'
        clean_target 'package-lock.json'
    fi
fi
...
npm run build
echo "==> building the eitb entry point"
npm run eitb:build
